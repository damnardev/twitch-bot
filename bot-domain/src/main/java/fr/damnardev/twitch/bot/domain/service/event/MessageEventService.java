...
public class MessageEventService implements IMessageEventService {
    ...
    public void process(MessageEvent event) {
        var channel = findChannelRepository.find(event.user()).orElse(null);
        ...
        var commandConfiguration = findCommandConfigurationRepository.find(command);
        if (!commandConfiguration.enabled() || !commandConfiguration.canProcess()) {
            return;
        }
        var result = commands.get(command.name()).process(event);
        var message = toMessage(user, result);
        saveCommandConfigurationRepository.updateLastExecution(commandConfiguration);
